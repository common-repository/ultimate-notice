( function ( $ ) {
	...
	$( document ).ready( function () {
		var notice = $( '#ultimate-notice' ),
			cookie = $.fn.getCookieNotice();

		// handle set-cookie button click
		$( document ).on( 'click', '.ultimate-notice-set-cookie', function ( e ) {
			e.preventDefault();
		...
		$( document ).on( 'click', '.ultimate-notice-revoke-cookie', function ( e ) {
			e.preventDefault();
			...
			if ( cnArgs.refuse === 'yes' ) {
				...
				if ( cnArgs.revoke_cookies === '1' ) {
					...
					if ( revoke.hasClass( 'ultimate-notice-revoke-inline' ) ) {
						var body = $( 'body' );

						// is ultimate notice hidden?
	...
	$.fn.setCookieNotice = function ( cookie_value ) {
		...
		if ( cookie_value && cnArgs.redirection === '1' ) {
			...
			if ( cnArgs.cache === '1' ) {
				url = url + hostname.replace( '//', '/' ) + ( window.location.search === '' ? '?' : window.location.search + '&' ) + 'ultimate-notice-reloaded=1' + window.location.hash;
	...
	$.fn.getCookieNotice = function () {
		var value = "; " + document.cookie,
			parts = value.split( '; ultimate_notice_accepted=' );
	...
	$.fn.showCookieNotice = function( type ) {
		...
		switch ( type ) {
			...
			case 1:
				if ( cnArgs.hideEffect === 'fade' ) {
					// hide revoke button
					notice.find( '.ultimate-notice-revoke-container' ).fadeOut( 400, function () {
						// show ultimate notice
						notice.css( { 'visibility': 'visible', 'display': 'none' } ).fadeIn( 400 );
					} );
				} else if ( cnArgs.hideEffect === 'slide' ) {
					// hide revoke button
					notice.find( '.ultimate-notice-revoke-container' ).slideUp( 400, function () {
						// show ultimate notice
						notice.css( { 'visibility': 'visible', 'display': 'none' } ).slideDown( 400 );
					} );
				} else {
					// hide revoke button
					notice.css( { 'visibility': 'visible' } ).find( '.ultimate-notice-revoke-container' ).hide();
				}
				break;
	...
	$.fn.hideCookieNotice = function ( type ) {
		...
		switch ( type ) {
			...
			case 1:
				if ( cnArgs.hideEffect === 'fade' ) {
					notice.find( '.ultimate-notice-revoke-container' ).hide().fadeIn( 400 ).css( { 'visibility': 'visible', 'display': 'block' } );
				} else if ( cnArgs.hideEffect === 'slide' ) {
					notice.find( '.ultimate-notice-revoke-container' ).hide().slideDown( 400 ).css( { 'visibility': 'visible', 'display': 'block' } );
				} else {
					notice.find( '.ultimate-notice-revoke-container' ).css( { 'visibility': 'visible', 'display': 'block' } );
				}
				break;
			...
			case 2:
				if ( cnArgs.hideEffect === 'fade' ) {
					notice.fadeOut( 400, function () {
						// show revoke button
						notice.css( { 'visibility': 'hidden', 'display': display } ).find( '.ultimate-notice-revoke-container' ).hide().fadeIn( 400 ).css( { 'visibility': 'visible', 'display': 'block' } );
					} );
				} else if ( cnArgs.hideEffect === 'slide' ) {
					notice.slideUp( 400, function () {
						// show revoke button
						notice.css( { 'visibility': 'hidden', 'display': display } ).find( '.ultimate-notice-revoke-container' ).hide().slideDown( 400 ).css( { 'visibility': 'visible', 'display': 'block' } );
					} );
				} else {
					// show revoke button
					notice.css( { 'visibility': 'hidden' } ).find( '.ultimate-notice-revoke-container' ).css( { 'visibility': 'visible', 'display': 'block' } );
				}
				break;
		}
	}
